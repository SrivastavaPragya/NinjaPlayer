    ...
    <>
      <div   className='flex justify-between p-3 border-b-[2px] border-[#FF3366]'>
        ...
        <div className='flex gap-4'>
          <button onClick={()=>router.push('/create-post')} className='bg-black p-1 px-3 text-white rounded-full text-[12px]'>CREATE POST</button>
          {session ? (
          ...
          <Image onClick={()=>router.push('/profile')} className='rounded-full cursor-pointer' src={ session?session?.user?.image:USER_IMAGE} alt='user image' width={40} height={40} />
        </div>
      </div>
    </>
  )
